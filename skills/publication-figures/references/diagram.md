...
`assets/figure-tikz-preamble.tex` defines the block, data, arrow, and stage
styles and the same palette the plots use. Resolve the asset path relative to
this skill's `SKILL.md`, then copy it to `figs/method.tex` as the head of the
figure source. Do not depend on a separate source checkout.
...
```latex
% ... contents of figure-tikz-preamble.tex above this line ...
...
`\familydefault` across — the sans face rides in the `figure`, `edgelbl`, and
